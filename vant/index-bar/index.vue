...
<view class="van-index-bar">
  ...
  <view v-if="showSidebar" class="van-index-bar__sidebar" @tap.stop="onClick" catch:touchmove="onTouchMove" catch:touchend="onTouchStop" catch:touchcancel="onTouchStop">
    <view v-for="(item, index) in indexList" :key="index" class="van-index-bar__index" :style="'z-index: ' + ( zIndex + 1 ) + '; color: ' + ( activeAnchorIndex === index ? highlightColor : '' )" :data-index="index">
      {{ item }}
    </view>
  </view>
</view>
...
export default {
  ...
  methods: {
    ...
    scrollToAnchor(index) {
      ...
      const anchor = this.children.find(item => item.data.index === this.indexList[index]);
